...
author: "Grupo laboratorio: María del Rocio Vidal | Javier Martínez Germán | Óscar Jiménez Osuna."
date: "31/01/2021"
output:
  html_document: 
    df_print: default
...
### Número de días en los que no han existido suspensiones.

```{r}
sum(df_2008$TOTAL_TRAB_SUS==0)
sum(df_2020$TOTAL_TRAB_SUS==0)
```

# Modelo 1. Modelo relacional basado en la estadística relacional básica.
...
### Tabla de frecuencias relativas por sectores.

```{r}
sectores_2008 = c(list(df_2008$SECTOR))
sectores_2020 = c(list(df_2020$SECTOR))
prop.table(table(sectores_2008))*100
prop.table(table(sectores_2020))*100
```

### Diagrámas de cajas por sectores.

```{r}
boxplot(df_2008$TOTAL_TRAB_SUS~df_2008$SECTOR, main="Diagrama de caja de trabajadores extinguidos año 2008 por sectores", ylab="nº trabajadores extinguidos", xlab="",col = "red", ylim=c(0,35), cex.axis = 0.7)
boxplot(df_2020$TOTAL_TRAB_SUS~df_2020$SECTOR, main="Diagrama de caja de trabajadores extinguidos año 2020 por sectores", ylab="nº trabajadores extinguidos", xlab="",col = "red", ylim=c(0,30), cex.axis = 0.7)
```
